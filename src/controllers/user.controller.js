...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    await user.save({ validateBeforeSave: false }); //used for saving the user as this is come from mongodb so the schema parameters like username , password, etc kickin (activate) and they become rquired so to save the user without parameters we use validateBeforeSave : false
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  let coverImageLocalPath; //checking  cover image is uploaded or not
  if (
    req.files &&
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!username && !email) {
    //for both username or email login any
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  const avatarLocalPath = req.file?.path; //file is getting by multer
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  const coverImageLocalPath = req.file?.path; //file is getting by multer
...
const getUserchannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    {
      $match: {
        username: username?.toLowerCase(), //find user
      },
    },
    {
      $lookup: {
        from: "subscriptions", //check subscription of user
        localField: "_id",
    ...
    {
      $lookup: {
        from: "subscriptions", //check subscriber of the channel
        localField: "_id",
